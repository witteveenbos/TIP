...
from hail.models.request import MunicipalityScenario, PostUserInputRequest
...
def mock_context(request: PostUserInputRequest, accessed_attributes: AccessedAttributes) -> ContextProvider:
    return ContextProvider(
        MockMultiScenarioDataWrapper(
            num_scenarios=len(request.userSettings.municipalityScenarios),
            actual_ids=[ms.ETMscenarioID for ms in request.userSettings.municipalityScenarios],
        ),
...
async def initial_context_call(
    ...
    municipality_scenarios: list[MunicipalityScenario] = request.userSettings.municipalityScenarios

    # If single graph focus is requested, filter the scenarios to only include the relevant municipality
    if (
        isinstance(request.viewSettings.graphFocus, MunicipalityIDs)
        and request.viewSettings.graphType is GraphTypes.ENERGYBALANCE_CURVE
    ):
        municipality_scenarios = [
            ms for ms in municipality_scenarios if ms.municipalityID is request.viewSettings.graphFocus
        ]
    ...
    needs_curves = (
        request.viewSettings.graphType is not None and request.viewSettings.graphType.value == "energybalance_curve"
    )
...
def get_palettes():
    ...
    palettes = [(name, getattr(cc, name)) for name in cc.__dir__() if name.startswith("b_")]
...
def get_mapclasses(configpath: Path) -> list[AbstractResultMap]:
    ...
    classes: list[AbstractResultMap] = import_all_classes_from_folder(configpath, AbstractResultMap)
    return classes


def get_developmentclasses(configpath: Path) -> list[AbstractDevelopment]:
    classes: list[AbstractDevelopment] = import_all_classes_from_folder(configpath, AbstractDevelopment)
    return classes
...
async def get_main_graph(
    ...
    energy_balance_graph = [graph for graph in preloaded.graphclasses if graph.key == "energybalance_bar"][0]
...
def log_reason_postuservalues(request: PostUserInputRequest, changes_by_user: bool):
    if request.viewSettings.original is True:
        logging.info("[app:post_user_values] Returning original results because viewSettings.original is True")
    elif not changes_by_user:
        logging.info("[app:post_user_values] Returning original results because no changes by user")
    else:
        logging.info("[app:post_user_values] Returning updated results because changes by user")
...
async def get_gquery_value(
    ...
    try:
        ...
        if raw_value.dimension == 2:
            ...
            logging.debug(
                f"[get_gquery_value] Converted 2D Matrix (timeseries) to data: shape={len(value)}x{len(value[0]) if value else 0}"
            )
        else:
...
async def get_curve_graph(
    ...
    accessed_attributes = preloaded.accessed_attributes
    scenario_relations = preloaded.scenario_relations
    energy_balance_curve_graph = [graph for graph in preloaded.graphclasses if graph.key == "energybalance_curve"][0]

...
excel_path = "20260204_scenario_links.xlsx"
output_path = "20260204_scenario_links_fixed.xlsx"
...
for sheet_name in excel_file.sheet_names:
    ...
    # Assuming the municipality name is in the first column (index 0)
    for i, row in df.iterrows():
        location = str(row[0]).replace("KM_", "")
        ...
        if municipality_name in municipality_mapping:
            df.loc[i, 0] = municipality_mapping[municipality_name]
        else:
